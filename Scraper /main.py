...
url: https://github.com/MatthewChatham/glassdoor-review-scraper

Although, the core of the scraper is the same, due to some updates on Glassdoor website, I had to adjust some paths.
Also,    
'''
...
def scrape(field, review, author):
    ...
    def scrape_date(review):
        ...
        #print(review.find_element_by_tag_name(
        #    'time').get_attribute('datetime'))
        return review.find_element_by_tag_name(
            'time').get_attribute('datetime')

    def scrape_emp_title(review):
        logger.info("scraping employee title")
    ...
    def scrape_main_text(review):
        logger.info("scraping main text")
        try:
            #driver.findElementByCssSelector("mainText mb-0").click()
            #main_text = review.find_element_by_class_name('row mt').text
            main_text = review.find_element_by_css_selector('p.mainText').text
            res = main_text.strip()
        except Exception:
            logger.info("scrape_main_text: exception")
            res = np.nan
        #print(res)
        return res

    def scrape_rev_title(review):
        logger.info("review title")
        #print(review.find_element_by_class_name('summary').text.strip('"'))
        return review.find_element_by_class_name('summary').text.strip('"')

    def expand_show_more(section):
        logger.info("expanding: continue reading")
        try:
            ...
            more_link.click()
            logger.info("expanding: success")

        except Exception:
            pass

    def scrape_pros(review):
        logger.info("scraping pros")
        try:
    ...
    def scrape_overall_rating(review):
        logger.info("scraping rating")
        try:
            ratings = review.find_element_by_class_name('v2__EIReviewsRatingsStylesV2__ratingInfo').text
            res = ratings
        except Exception:
            logger.info("scrape_overall_rating: exception")
            res = np.nan
        #print(res)
        return res
